  ...
  useEffect(() => {
    if (isOpen && !initialized.current && session.sessionId) {
      ...
      loadCheckoutScript()
        ...
        .catch((error) => console.error('Failed to load Checkout script:', error));
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="relative bg-white rounded-2xl shadow-2xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-hidden">
        <div style={{ background: 'linear-gradient(to right, var(--navy2), var(--navy))' }} className="px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm" style={{ color: 'rgba(255,255,255,0.8)' }}>Powered by Mastercard</p>
            </div>
          </div>
          <button onClick={onClose} className="text-white/80 hover:text-white transition-colors p-2 rounded-lg hover:bg-white/10" aria-label="Close">
        ...
        <div className="px-6 py-4 border-b border-gray-200" style={{ background: 'var(--light)' }}>
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              <p className="text-gray-600 mb-1">Amount</p>
              <p className="text-2xl font-bold" style={{ color: 'var(--navy)' }}>{currency} {amount.toLocaleString()}</p>
            </div>
            <div>
        ...
        <div className="p-6 bg-white overflow-y-auto" style={{ maxHeight: '500px' }}>
          <div id="mastercard-payment-container" ref={paymentContainerRef} className="min-h-[400px]" />
        </div>
